...
{ 
  ...
  if ($('error')) {
    new Effect.Pulsate('error', {pulses: 2, duration: 1});
    setTimeout("new Effect.Fade('error');", 6000)
  }
  new accordion('post_misc')
}

function expand(e)
{
  el = $('whatis_expander').up().next();
  if (el.className='expand')
    new Effect.BlindDown(el);
}
Event.observe(window, 'load', initGvido, false);