class Invoice < ActiveRecord::Base
  ...
  class << self
    ...
    # Returns saved (settled) invoices for date
    #
    def on_date(date)
      ...
      if payment_dates.any?
        for payment_day in payment_dates
          ...
          if saved_invoices.any?
            new_invoices = self.new_on_date(payment_day)
            ...
            unsettled += self.new_on_date(payment_day)
          end
        end
      end
    ...
    # 
    def invoices_expected_settled_and_unsettled_sum_on_date(date)
      saved_invoices = self.on_date(date)

      if saved_invoices.any?
        new_invoices = self.new_on_date(date)
        saved_monthly_references = saved_invoices.collect(&:monthly_reference)
        ...
        all_invoices = Invoice.new_on_date(date)
        expected_invoices_sum  = all_invoices.map(&:price).sum
